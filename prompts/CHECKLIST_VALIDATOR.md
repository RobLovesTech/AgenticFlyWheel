...
1) Structure
  ...
  - If `requirements_mode` is present, it must be `technical`, `operating`, or `mixed`.
  - If `packet_level` is absent, fall back to legacy heuristics instead of failing solely on the missing field.
  - If `delivery_surfaces` is present, each surface should be one of: `backend`, `frontend`, `data`, `contracts`, `docs`, `integrations`, `operations`, `gtm`, `client_rollout`, `enablement`, `support`, `governance`, `compliance`.
  - If `enabled_modules` is present, each module must be one of: `contracts`, `data_model`, `backend_implementation`, `orchestration_and_ui`, `observability`, `runbook`, `operating_model`, `gtm_and_launch`, `client_rollout`, `enablement_and_support`, `governance_and_approvals`.
...
2) Required phase
  ...
  - A readiness-evidence task exists before packet closure.
    - Prefer task id `readiness-evidence`.
    - This task should record operating readiness evidence, approvals, training/comms completion, support ownership, or an explicit not-applicable rationale when those surfaces are out of scope.
  ...
  - The final phase includes:
    ...
    - A task to record readiness evidence or explicit not-applicable rationale before closure.
  ...
  - For manifest-driven full AIPs, module-specific tasks and phase references should align with `enabled_modules`.
     ...
     - If `operating_model` is omitted, do not require `OPERATING_MODEL.md` tasks or references.
     - If `gtm_and_launch` is omitted, do not require `GTM_AND_LAUNCH.md` tasks or references.
     - If `client_rollout` is omitted, do not require `CLIENT_ROLLOUT.md` tasks or references.
     - If `enablement_and_support` is omitted, do not require `ENABLEMENT_AND_SUPPORT.md` tasks or references.
     - If `governance_and_approvals` is omitted, do not require `GOVERNANCE_AND_APPROVALS.md` tasks or references.
...
5) Collaboration receipt completeness
   ...
   - Require `Operating Readiness Decisions` when `requirements_mode` is `operating` or `mixed`, when `delivery_surfaces` include any of `operations`, `gtm`, `client_rollout`, `enablement`, `support`, `governance`, `compliance`, or when operating modules are enabled.
...
6) Optional (recommend)
   - New full AIPs include `packet_level`, `requirements_mode`, `delivery_surfaces`, `enabled_modules`, and `omitted_modules` in `CHECKLIST.yaml`.
