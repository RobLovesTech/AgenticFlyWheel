...
3. Prefer these sources:
   ...
   - verification evidence from `CHECKLIST.yaml` and `RUNBOOK.md` when the `runbook` module is enabled
4. Confirm all non-audit implementation, verification, docs-sync, and prompt-artifact tasks are complete enough to audit, using the packet manifest to determine which optional docs should exist.
...
9. Refresh `AGENT_PROMPT.txt` and `IMPLEMENTATION_AUDIT_PROMPT.txt` if audit-driven packet changes affect execution or handoff instructions.
